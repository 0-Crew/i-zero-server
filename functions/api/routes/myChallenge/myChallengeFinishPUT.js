...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let myChallenge = await myChallengeDB.getMyChallengeById(client, myChallengeId);
    if (!myChallenge) return res.status(404).json({ err: true, userMessage: 'myChallengeId is not valid' });

    myChallenge = await myChallengeDB.finishMyChanllengeByMyChallengeId(client, myChallengeId);
